...
static void requestMoreSpace(size_t reqSize) {
  ...
  *((size_t*)UNSCALED_POINTER_ADD(newBlock, totalSize)) = TAG_USED;
  ...
  //printf("MORE SPACE SUCCESSFULLY RETRIEVED. PTR: %p\n", newBlock);
...
void* mm_malloc (size_t size) {
  ...
  //If the free list search returns null, request more space and search again
  ...
  //capture the blocksize and preceding block used tag
  ...
  if((blockSize - reqSize) >= MIN_BLOCK_SIZE){
    ...
    //used for splitting off unecessary space in allocation block
    BlockInfo * ptrSplitFreeBlock = UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);

    //captures the difference between the amount needed and amount
    //returned from free list search
    size_t sizeDiff = blockSize - reqSize;

    //update the allocated block size to be the required size instead
    //of size of entire block
    ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag;

    //update the split block size and tags header and footer
    ptrSplitFreeBlock->sizeAndTags = sizeDiff | TAG_PRECEDING_USED;

    ((BlockInfo*) UNSCALED_POINTER_ADD(ptrSplitFreeBlock, sizeDiff-WORD_SIZE))->sizeAndTags = sizeDiff | TAG_PRECEDING_USED;
  ...
  //Go to the word after the newly allocated block and set the tag preceding bit to 1
  ptrNextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, SIZE(ptrFreeBlock->sizeAndTags));
  ptrNextBlock->sizeAndTags = ptrNextBlock->sizeAndTags | TAG_PRECEDING_USED;
  
  //set the malloc block used bit to 1
...
void mm_free (void *ptr) {
  ...
  //get to header of block being freed and capture SIZE
  blockInfo = UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  blockSize = SIZE(blockInfo->sizeAndTags);

  //printf("FREEING AT LOC: %p\nSIZE: %zu\n\n", blockInfo, blockSize);
  
  //set header & boundary  of block used bit to 0
  ...
  //capture following block and set the preceding used bet to 0
  ...
  //insert and coalesce if necessary
